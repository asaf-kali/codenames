...
class DuetSideState(DuetSpymasterState):
    ...
    def assassin_hit(self) -> bool:
        return any(guess.guessed_card.color == DuetColor.ASSASSIN for guess in self.given_guesses)
    ...
    def _quit(self):
        self.game_result = QUIT
        self._end_turn()

    def _update_score(self, given_guess: DuetGivenGuess):
...
class DuetGameState(BaseModel):
    ...
    def from_board(cls, board: DuetBoard) -> DuetGameState:
        ...
        side_a = DuetSideState.from_board(board)
        side_b = DuetSideState.from_board(dual_board)
    ...
    def game_result(self) -> GameResult | None:
        ...
        if not result_a and not result_b:
            return None
        # If any side has lost, the game is list
    ...
    def process_guess(self, guess: Guess) -> DuetGivenGuess | None:
        side_state = self.current_side_state
        given_guess = side_state.process_guess(guess)
